...
class TaskComposeViewController: UIViewController {
    ...
    @IBAction func didTapDoneButton(_ sender: Any) {
        ...
        if let editTask = taskToEdit {
            ...
            print("\n\n\n we are printing its id \n\n\n")
            print(editTask.id)
            if let index = currentTasks.firstIndex(where: { $0.title == editTask.title }) {
                currentTasks.remove(at: index) // Remove the editTask from the currentTasks array
                currentTasks.insert(task, at: index)
                Task.save(currentTasks, forKey: Task.tasksKey) // Save the updated tasks
            }
        
            //task.addToCurrentTasks()
