...
class TaskListViewController: UIViewController {
    ...
    @IBOutlet weak var calendarSegueButton: UIButton!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        calendarSegueButton.setTitle("I want to see a calendar so that I can admire myself by fantasizing about my future achievements and the dates on which I will complete them", for: .normal)
        calendarSegueButton.titleLabel?.numberOfLines = 0
        calendarSegueButton.titleLabel?.lineBreakMode = .byWordWrapping
...
extension TaskListViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            // Remove the associated task from your data source
            let taskToRemove = tasks[indexPath.row]
            
            // Call your removeFromCurrentTasks() function to remove the task from the current tasks and save the updated array
            taskToRemove.removeFromCurrentTasks()
            
            // Update the table view's data source
            tasks = Task.getTasks(forKey: Task.tasksKey)
            
            // Tell the table view to delete the associated row
